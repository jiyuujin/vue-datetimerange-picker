...
export default Vue.extend({
  ...
  computed: {
    ...
    calendar(): Array<Array<number | Date>> {
      ...
      const lastMonth = DateUtil.prevMonth(firstDay).getMonth() + 1
      const lastYear = DateUtil.prevMonth(firstDay).getFullYear()
      const daysInLastMonth = new Date(lastYear, month - 1, 0).getDate()

      const dayOfWeek = firstDay.getDay()

      const calendar: Array<Array<number | Date>> = []
    ...
    months(): Array<{
      ...
      const monthsData = this.monthList.map((m) => ({
        text: m.text,
        value: m.value
      }))
  ...
  methods: {
    ...
    changeMonthDate(date: Date, emit = true) {
      const yearMonth = DateUtil.yearMonthFormat(this.currentMonthDate)
